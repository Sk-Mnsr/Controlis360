...
router.beforeEach(async (to, from, next) => {
    ...
    if (to.matched.some((record) => record.meta.canCreateMission)) {
        if (!canCreateMission(auth.user)) {
            return next({ name: 'audit.missions' });
        }
    }

    const moduleSlug = to.matched.find((record) => record.meta.module)?.meta.module;
    if (moduleSlug && !canAccessModule(auth.user?.profile, moduleSlug, auth.user)) {
        return next({ name: 'portal' });
    }
